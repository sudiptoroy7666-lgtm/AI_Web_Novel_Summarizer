...
> AI-powered reading assistant — browse, extract, and summarise web novel chapters with one tap
...
| Feature | Description |
|---|---|
| 🌐 Smart WebView Browser | Full browser with URL detection, search routing, back/forward, refresh |
| 📚 6+ Pre-configured Novel Sites | One-tap access to WebNovel, WuxiaWorld, RoyalRoad, BoxNovel, and more |
| 🤖 3-Level AI Summaries | Short (bullet points), Detailed (paragraphs), Very Detailed (immersive) |
| 🚫 15+ Domain Ad Blocker | Blocks ad networks, trackers, popups, and cookie banners |
| 🌙 Light/Dark Mode CSS Injection | Dark and light reading modes injected via JavaScript |
| 📂 Novel Library (3-tier) | Novels → Volumes → Chapters hierarchy with full CRUD |
| 🔊 Text-to-Speech | Chunked TTS with play/pause/stop and progress tracking |
| 🕒 Browsing History | Auto-tracked page visits with swipe-to-delete |
| ⭐ Bookmarks | One-tap save with duplicate detection |
| 🎯 Manual CSS Selector | Override extraction for problematic pages |
| 📴 Offline Access | All saved summaries accessible without internet |
| 🔁 Cerebras + Groq Fallback | Auto-switches to Groq if Cerebras rate-limits |
...
- **Primary AI:** Cerebras API — GPT-OSS-120B
- **Fallback AI:** Groq API — Llama 3.3 70B
...
├── ui/
│   ├── MainActivity.kt          (Browser)
│   ├── ActivitySummary.kt
│   ├── ActivityLibrary.kt
│   ├── ActivityVolumes.kt
│   ├── ActivityChapters.kt
│   ├── ActivityHistory.kt
│   └── ActivityBookmarks.kt
├── viewmodel/
│   ├── BrowserViewModel.kt
│   ├── SummaryViewModel.kt
│   └── LibraryViewModel.kt
└── util/
    └── ContentHolder.kt         (singleton for large text transfer)
```
...
Android's Intent has a 1MB transaction limit — novel chapters easily exceed this. A singleton `ContentHolder` stores content in memory and is accessed by the summary screen directly, bypassing Intent entirely.
...
**Chunked TTS**
The TTS engine splits summaries at sentence boundaries into ≤4,000-character chunks (the safe Android TTS limit), plays them sequentially, and maintains pause/resume state across chunk boundaries.
...
- **No content on third-party servers** — AI calls are direct API requests, content is not stored remotely
